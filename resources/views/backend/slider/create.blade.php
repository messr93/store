...
@section('content')
    <form action="{{ route('admin.slider.store') }}" method="post" enctype="multipart/form-data">@csrf
        ...
        <div class="form-group">
            ...
            <input type="text" id="title" name="title" class="form-control @error('title') is-invalid @enderror" placeholder="{{ __('backend.Slider title') }}" value="{{ old('title') }}">
            @error('title')
        ...
        <div class="form-group">
            <label for="link">{{ __('backend.Slider link') }}:</label>
            <input type="text" id="link" name="link" class="form-control @error('link') is-invalid @enderror" placeholder="{{ __('backend.Slider link') }}" value="{{ old('link') }}">
            @error('link')
            <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
